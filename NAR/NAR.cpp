...
void NAR::AddNewJob(const cv::Mat &img)
{
    ...
    if(img.channels() == 3) {  //Returns the number of matrix channels.
        cv::cvtColor(img, grey, CV_BGR2GRAY); //RGB convert to Gray
    }
    ...
    for(int i=0; i < KEYPOINT_LEVELS; i++) {
        ...
        if(i == 0) { // Original pic need no subpixel process
            new_job.grey = grey;
